...
class SiameseEncoder(nn.Module):
    def __init__(self, in_channels=3, output_feats=1024):
        super(SiameseEncoder, self).__init__()
        self.inplanes = 64
        self.siamese = nn.Sequential(convbn(in_planes=in_channels,
                                            out_planes=64,
                                            kernel_size=7,
                                            stride=2,
                                            pad=3,
                                            dilation=1),
                                     self._make_layer(
                                         BasicBlock, inplanes=self.inplanes, planes=128, blocks=2, stride=2, pad=1, dilation=1),
...
class RotationNet(nn.Module):
  def __init__(self, emb_feats = 1024, n_out=3, dropout = 0.5, use_perturb = False, in_channels=3):
    super(RotationNet,self).__init__()
    assert emb_feats % 2 == 0, "image embedding feature length must be multiple of 2"
    self.encoder = SiameseEncoder(in_channels=in_channels, output_feats=emb_feats//2)
    self.n_out = n_out
    ...
    if use_perturb:
        print("Using Sigmoid to bound the corrections")
        self.rot_regress.append(nn.Sigmoid())

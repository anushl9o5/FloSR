...
from utility.data_utils import calibrate_stereo_pair_torch, svd_orthogonalize
...
class HSMNet(nn.Module):
    ...
    def rectify_feats(self, feat_l, feat_r, rot, translation, intrinsics_left, intrinsics_right, dist1, dist2):
        ...
        feat_l_rectf, feat_r_rectf, _ = calibrate_stereo_pair_torch(
            feat_l,
            feat_r,
            intrinsics_left.double(),
            intrinsics_right.double(),
            dist1.double(),
            dist2.double(),
            rotation,
            translation.double(),
    ...
    def forward(
        self, left, right, translation, intrinsics_left, intrinsics_right, dist1, dist2, full_height, full_width
    ):
        ...
        for i in range(len(conv_l[0:1])):
            ...
            conv_l_rectf, conv_r_rectf = self.rectify_feats(
                conv_l[i],
                conv_r[i],
                R,
                translation,
                intrinsics_left * scale_mat,
                intrinsics_right * scale_mat,
                dist1,
                dist2,
            )
            feat_vol = self.feature_vol(
                conv_l_rectf, conv_r_rectf, self.maxdisp // levels[i]
            )  # self.get_cost_vol(conv_l_rectf, conv_r_rectf, hor_conv3d=self.conv3ds[i], pool=False, level=levels[i])
            ...
            R = (
                R + R_diff
            )  # torch.bmm(svd_orthogonalize(R_diff.reshape(-1, 3, 3)).permute(0, 2, 1).double(), svd_orthogonalize(R.reshape(-1, 3, 3))).reshape(-1, 9)

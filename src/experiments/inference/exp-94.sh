#!/bin/sh
export CUDA_VISIBLE_DEVICES=4,5
python src/infer.py \
	            --dataset               flyingthings \
	            --data_dir              /home/eos/workspace/nas.anush/flyingthings/cleanpass/frames_cleanpass \
                    --gt_dir                /home/eos/workspace/nas.anush/flyingthings/camera_data \
		    --splits_dir            src/data/split_files/flyingthings/ \
                    --log_dir               /nas/EOS/users/anush/logs \
                    --height                512 \
                    --width                 896 \
                    --exp_num               94 \
                    --exp_name              hsm_infer \
                    --exp_metainfo          flyingthings_9D_R6_costvol_of \
                    --batch_size            1 \
                    --num_workers           12 \
		    --pose_repr		    use_9D \
                    --rotation              1e+02 \
                    --learning_rate         1e-05 \
                    --model_name            hsm \
                    --num_epochs            120 \
                    --algolux_test_res_opt_flow     0.5 \
		    --constrain_roi         \
		    --epoch_visual_loss     0 \
		    --use_radial_flow_mask  \
		    --resize_orig_img       \
		    --use_seq_hsm           \
		    --use_superglue_eval    \
		    --use_dispfeatvol_loss  \
		    --use_opt_flow_loss     \
		    --multi_gpu_opt_flow    \
		    --superglue_weights     indoor \
		    --load_weights_folder  /nas/EOS/users/anush/logs/hsm_debug/exp-94_flyingthings_9D_R6_costvol_of/models/weights_71/ \



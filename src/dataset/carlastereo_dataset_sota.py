...
class CarlaStereoPoseDatasetSOTA(data.Dataset):
    def __init__(
        ...
        with open(os.path.join(frames_dir, "config_args.json")) as fh:
    ...
    def __getitem__(self, index):
        ...
        assert os.path.basename(left_frame_path) == os.path.basename(right_frame_path), (
            "Left and Right image names are not same"
        )
        ...
        sota = {
            "dnet": "/nas/EOS/users/anush/DirectionNet/checkpoints/pred_poses",  # "/nas/EOS/users/anush/DirectionNet/final_model/pred_poses",
        ...
        with open(pose_json_fpath) as fh:
        ...
        t = (t_opencv_cam1 - R @ t_opencv_cam0).reshape(
            3,
        )
        ...
        self.K = np.array([[focal_length, 0, self.full_width / 2], [0, focal_length, self.full_height / 2], [0, 0, 1]])
        ...
        _, _, _, _, _, roi_left, roi_right = cv2.stereoRectify(
            self.K, distCoeff1, self.K, distCoeff2, (self.full_width, self.full_height), R, t
        )
        ...
        if self.pose_repr in ["use_perturb", "use_perturb_6D", "use_perturb_quat", "use_yaw_perturb", "use_identity"]:
            left_img, right_img = calibrate_stereo_pair(
                left_img,
                right_img,

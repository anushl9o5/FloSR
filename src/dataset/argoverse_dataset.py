...
class ArgoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        left_frame_path, right_frame_path, calib_file, capture = self.data[index]
        ...
        if self.resize_orig_img:
            left_img = resize_img(left_img, 1035, 1232)
            right_img = resize_img(right_img, 1035, 1232)

            width_scale, height_scale = 1232.0/self.full_width, 1035.0/self.full_height
    ...
    def parse_calib(self, calib_file):
        self.pose_gt = {}
        calib_dict = json.load(open(calib_file))

        for cam in calib_dict['camera_data_']:
            if cam['key'] == 'image_raw_stereo_front_left':
                left_stereo = cam['value']
            if cam['key'] == 'image_raw_stereo_front_right':
                right_stereo = cam['value']

        self.pose_gt["K_02"], self.pose_gt["D_02"] = self.get_intrinsics(left_stereo)
        self.pose_gt["K_03"], self.pose_gt["D_03"] = self.get_intrinsics(right_stereo)

        self.pose_gt["R_03"], self.pose_gt["T_03"] = self.get_extrinsics(right_stereo)
        self.pose_gt["R_02"], self.pose_gt["T_02"] = self.get_extrinsics(left_stereo)

        self.get_relative_pose()
    
    def get_extrinsics(self, data_dict):
        rot_quat = data_dict['vehicle_SE3_camera_']['rotation']['coefficients']
        trans_vec = data_dict['vehicle_SE3_camera_']['translation']

        return Rotation.from_quat(rot_quat).as_matrix(), np.array(trans_vec)

    def get_intrinsics(self, data_dict):
        I = np.eye(3)
        I[0, 0] = data_dict['focal_length_x_px_']
        I[1, 1] = data_dict['focal_length_y_px_']
        I[0, -1] = data_dict['focal_center_x_px_']
        I[1, -1] = data_dict['focal_center_y_px_']

        return I, np.array(data_dict['distortion_coefficients_'])

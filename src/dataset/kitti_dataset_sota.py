...
class KITTIDatasetSOTA(data.Dataset):
    def __init__(
        self,
        pose_gt_dir,
        frames_dir,
        ...
        with open(self.datafile_path) as csv_file:
    ...
    def __getitem__(self, index):
        ...
        self.is_flip = self.is_train and random.random() > 0.5

        if self.is_flip:
            right_frame_path, left_frame_path, calib_file, date, capture = self.data[index]
        else:
            left_frame_path, right_frame_path, calib_file, date, capture = self.data[index]

        # read calibration data
        inputs["dataset"] = capture

        self.parse_calib(calib_file)
        R, t = (
            np.array(self.pose_gt["rmat"]).reshape(3, 3),
            np.array(self.pose_gt["tvec"]).reshape(
                3,
            ),
        )
        ...
        if self.is_flip:
            K_left, dist_left = np.array(self.pose_gt["K_03"]).reshape(3, 3), np.array(self.pose_gt["D_03"]).reshape(-1)
            K_right, dist_right = (
                np.array(self.pose_gt["K_02"]).reshape(3, 3),
                np.array(self.pose_gt["D_02"]).reshape(-1),
            )
        else:
            K_left, dist_left = np.array(self.pose_gt["K_02"]).reshape(3, 3), np.array(self.pose_gt["D_02"]).reshape(-1)
            K_right, dist_right = (
                np.array(self.pose_gt["K_03"]).reshape(3, 3),
                np.array(self.pose_gt["D_03"]).reshape(-1),
            )
        ...
        inputs["img_id"] = f"{img_id}_{date}_{capture}"

        sota = {
            "dnet": "/nas/EOS/users/anush/DirectionNet/kitti_experiments/pred_poses",  # "/nas/EOS/users/anush/DirectionNet/final_model/pred_poses",
            "vit": "/nas/EOS/users/anush/sota/3dv/output/kitti/pred_poses",
            "rpnet": "/nas/EOS/users/anush/sota/RPNet/rpnet/kitti_train_rpnetplus/pred_rotation",
            "fepe": "/nas/EOS/users/anush/sota/RPNet/rpnet/kitti_train_rpnetplus/pred_rotation",
        }

        for sota_name, sota_folder in sota.items():
            sota_pose = np.load(os.path.join(sota_folder, inputs["img_id"] + ".npy"))
            inputs[f"R_{sota_name}"] = Rotation.from_matrix(sota_pose[:3, :3]).as_matrix()
            inputs[f"axisangles_{sota_name}"] = (
                Rotation.from_matrix(sota_pose[:3, :3]).as_rotvec(degrees=True).astype(np.float32)
            )
        ...
        R1, R2, P1, P2, Q, roi_left, roi_right = cv2.stereoRectify(
            K_left, dist_left, K_right, dist_right, (self.full_width, self.full_height), R, t
        )
        ...
        if self.pose_repr in ["use_perturb", "use_perturb_6D", "use_perturb_quat", "use_yaw_perturb", "use_identity"]:
            left_undist_img, right_undist_img, _ = calibrate_stereo_pair(
                left_undist_img,
                right_undist_img,
        ...
        with open(calib_file) as f:
            for line in f.readlines():
                if ":" in line:
                    ...
                    if key == "calib_time":
                        continue
                    self.pose_gt[key] = np.array([float(x) for x in value.split()])
    ...
    def get_relative_pose(self):
        ...
        if self.is_flip:
            relative_pose = np.linalg.inv(relative_pose)

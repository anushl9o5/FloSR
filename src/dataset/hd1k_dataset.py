...
class HD1KDataset(data.Dataset):
    ...
    def __init__(self, pose_gt_dir,
        ...
        super(HD1KDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        left_frame_path, right_frame_path, calib, capture, is_flipped = self.data[index]
        
        # read calibration data
        inputs['dataset'] = capture

        self.parse_calib(calib, os.path.basename(left_frame_path), is_flipped)
        ...
        K_left = np.array(self.pose_gt["K_101"]).reshape(3,3)
        K_right = np.array(self.pose_gt["K_103"]).reshape(3,3)
        ...
        left_img, right_img, _ = calibrate_stereo_pair(left_img, right_img, 
                                                       ...
                                                       Rotation.from_euler('xyz', R1_I, degrees = True).as_matrix(), t.astype(np.float64), 
                                                       self.full_height, self.full_width)

        T_gt = invert_transformation(get_transformation_matrix(Rotation.from_euler('xyz', R1_I, degrees = True).as_matrix(), t[:, None].astype(np.float64)))        
        ...
        if self.resize_orig_img:
            resize_width, resize_height = 1280, 512
        ...
        T21 = invert_transformation(T2_w)@T1_w
        R, t = T21[:3, :3], T21[:3, 3:].reshape(3,)

        if is_flipped == 'True':
            right_undist_img, left_undist_img = unrectify_np(right_undist_img, left_undist_img, 
                                            K_left, K_right,
        ...
        print("T1", T1_w)
        print("T2", T2_w)
        print("T_rect", self.pose_gt["T_rect"])
        '''
    ...
    def parse_extrinsics(self, input_string):
        values_str = re.search(r'\[(.*?)\]', input_string).group(1)
        ...
        # Reshape the values into a 3x3 numpy array
        intrinsics_matrix = np.array(intrinsics_matrix).reshape(3, 3)
    ...
    def get_intrinsics(self, filename):
        with open(filename) as f:
            ...
            cam0_line = lines_clean[0]
            cam1_line = lines_clean[1]
            
            self.pose_gt["K_101"] = self.parse_extrinsics(cam0_line)
            self.pose_gt["K_103"] = self.parse_extrinsics(cam1_line)


    def parse_calib(self, calib_path, img_id, is_flipped):
        self.pose_gt = {}

        self.get_intrinsics(calib_path)

        self.get_relative_pose(is_flipped)
    
    def get_relative_pose(self, is_flipped):

        relative_pose = np.eye(4) #np.linalg.inv(T_02)@T_03
        relative_pose[0, 3] = 0.3

        if is_flipped == 'True':
            self.pose_gt["T_rect"] = np.linalg.inv(relative_pose)
        else:

...
class SintelDataset(data.Dataset):
    ...
    def cam_read(self, filename):
        ...
        self.pose_gt["K_101"] = M
        self.pose_gt["K_103"] = M

        self.pose_gt["R_101"] = N[:3, :3]
        self.pose_gt["R_103"] = N[:3, :3]
        self.pose_gt["t_101"] = N[:3, 3]
        self.pose_gt["t_103"] = N[:3, 3] + np.array([0.1, 0, 0]) #Mentioned in Website, right cam is translated 0.1cm to the right
    ...
    def parse_calib(self, calib_path, img_id, is_flipped):
        ...
        self.cam_read(calib_path)

        self.get_relative_pose(is_flipped)
    
    def get_relative_pose(self, is_flipped):
        R_02 = self.pose_gt["R_101"].reshape(3, 3)
        t_02 = self.pose_gt["t_101"].reshape(3, 1)
        R_03 = self.pose_gt["R_103"].reshape(3, 3)
        t_03 = self.pose_gt["t_103"].reshape(3, 1)

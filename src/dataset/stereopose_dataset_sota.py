...
class StereoPoseDatasetSOTA(data.Dataset):
    def __init__(
        self,
        pose_gt_dir,
        frames_dir,
    ...
    def __getitem__(self, index):
        ...
        dataset, week, capture, cam_type, target_cam_num, ref_cam_num, left_frame_path, right_frame_path = self.data[
            index
        ]
        ...
        R, t = (
            np.array(pose["rmat"]).reshape(3, 3),
            np.array(pose["tvec"]).reshape(
                3,
            ),
        )
        ...
        sota = {
            "dnet": "/home/eos/workspace/code/DirectionNet/torc_experiments/pred_poses",  # "/nas/EOS/users/anush/DirectionNet/final_model/pred_poses",
            "vit": "/home/eos/workspace/code/sota/3dv/output/torc/pred_poses",
            "rpnet": "/home/eos/workspace/code/sota/RPNet/rpnet/torc_train_rpnetplus/pred_rotation",
            "fepe": "/home/eos/workspace/code/sota/RPNet/rpnet/torc_train_rpnetplus/pred_rotation",
        }
        ...
        for sota_name, sota_folder in sota.items():
            sota_pose = np.load(
                os.path.join(
                    sota_folder, f"{dataset}_{week}_{capture}_{img_id}_{target_cam_num}_{ref_cam_num}" + ".npy"
                )
            )
        ...
        _, _, _, _, _, roi_left, roi_right = cv2.stereoRectify(
        ...
        if self.pose_repr in ["use_perturb", "use_perturb_6D", "use_perturb_quat", "use_yaw_perturb", "use_identity"]:
            left_undist_img, right_undist_img = calibrate_stereo_pair(
                left_undist_img,
                right_undist_img,

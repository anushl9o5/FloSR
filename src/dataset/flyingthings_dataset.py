...
class FlyingThingsDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        self.parse_calib(calib, os.path.basename(left_frame_path))
        ...
        if self.resize_orig_img:
            resize_width, resize_height = 896, 512
    ...
    def parse_calib(self, calib_path, img_id):
        self.pose_gt = {}
        ...
        try:
            self.get_extrinsics(calib_dict[img_id])
        except KeyError:
            print(calib_path)
            exit()
    ...
    def get_extrinsics(self, data_dict):
        T_101 = np.array([float(x) for x in data_dict['L']]).reshape(4, 4)
        T_103 = np.array([float(x) for x in data_dict['R']]).reshape(4, 4)

        self.pose_gt["R_101"] = T_101[:3, :3]
        self.pose_gt["R_103"] = T_103[:3, :3]        

        self.pose_gt["t_101"] = T_101[:3, 3:]
        self.pose_gt["t_103"] = T_103[:3, 3:]

        self.pose_gt["T_101"] = T_101
        self.pose_gt["T_103"] = T_103

    def get_intrinsics(self):
        self.pose_gt["K_101"] = np.array([[1050.0, 0.0, 479.5], [0.0, 1050.0, 269.5], [0.0, 0.0, 1.0]])
        self.pose_gt["K_103"] = np.array([[1050.0, 0.0, 479.5], [0.0, 1050.0, 269.5], [0.0, 0.0, 1.0]])
    ...
    def opencv_pose_estimation(self, img1, img2, K1, K2):
        ...
        try:
            points, R_est, _, mask_pose = cv2.recoverPose(E, pts_l_norm[mask==1], pts_r_norm[mask==1])
        except:

...
import numpy as np
import torch
from kornia.geometry import essential_from_fundamental, find_fundamental, ransac
...
def svd_orthogonalize(m):
    ...
    r = torch.matmul(
        torch.concat([v[:, :, :-1], v[:, :, -1:] * torch.reshape(det, [-1, 1, 1])], 2), torch.transpose(u, 1, 2)
    )
...
def calibrate_stereo_pair(img_left, img_right, K_left, K_right, dist_left, dist_right, R, t, height, width):
    ...
    R1, R2, P1, P2, Q, roi_left, roi_right = cv2.stereoRectify(
        K_left, dist_left, K_right, dist_right, (width, height), R, t, flags, alpha=-1
    )
...
def undistortrectify_cv2(img_left, img_right, K_left, K_right, dist_left, dist_right, R1, R2, P1, P2, height, width):
    maplx, maply = cv2.initUndistortRectifyMap(
        K_left, dist_left, R1, newCameraMatrix=K_left, size=(width, height), m1type=cv2.CV_32FC1
    )

    maprx, mapry = cv2.initUndistortRectifyMap(
        K_right, dist_right, R2, newCameraMatrix=K_right, size=(width, height), m1type=cv2.CV_32FC1
    )
...
def get_rectangles_like_cv2(
    cameraMatrix: torch.Tensor,
    distCoeff: torch.Tensor,
    new_cameraMatrix: torch.Tensor,
...
def stereorectify_torch(
    cameraMatrix1: torch.Tensor,
    distCoeff1: torch.Tensor,
    cameraMatrix2: torch.Tensor,
    distCoeff2: torch.Tensor,
    img_size: Tuple[np.uint16, np.uint16],
    R: torch.Tensor,
    T: torch.Tensor,
    new_image_size: Optional[Tuple[np.uint16, np.uint16]] = (0, 0),
):
    ...
    r_r = axis_angle_to_rotation_matrix(om)  # rotate cameras to same orientation by averaging
    t = r_r @ T.clone()
    ...
    uu = (
        torch.scatter(uu.squeeze(2), dim=1, index=idx.reshape(-1, 1), src=torch.where(c > 0, 1.0, -1.0))
        .unsqueeze(2)
        .double()
    )
    ...
    fc_new = (
        torch.gather(
            torch.diagonal(cameraMatrix1, dim1=-2, dim2=-1), 1, torch.bitwise_xor(idx, torch.ones_like(idx)).view(-1, 1)
        ).squeeze(1)
        + torch.gather(
            torch.diagonal(cameraMatrix2, dim1=-2, dim2=-1), 1, torch.bitwise_xor(idx, torch.ones_like(idx)).view(-1, 1)
        ).squeeze(1)
    ) * ratio
    ...
    for k in range(2):  # k = 0 is left camera and k = 1 is left camera
        ...
        pts = (
            torch.Tensor([[0, 0], [nx - 1, 0], [0, ny - 1], [nx - 1, ny - 1]]).repeat(b, 1, 1).to(cameraMatrix1.device)
        )
...
def undistortrectify(
    image: torch.Tensor, K: torch.Tensor, dist: torch.Tensor, R: torch.Tensor, new_K: Optional[torch.Tensor] = None
) -> torch.Tensor:
    ...
    out = remap(
        image.reshape(B, channels, rows, cols),
        mapx.type_as(image),
        mapy.type_as(image),
        align_corners=True,
    )
...
def estimate_pose_kornia(
    kpts0, kpts1, K0, K1, mconf, thresh=1.0, scale=None, filter_height=-1, filter_conf=-1, top_8=False
):
...
def make_matching_plot_fast(image0, image1, kpts0, kpts1, mkpts0, mkpts1, color, margin=10, small_text=[]):
...
def find_sift_match(torch_img1, torch_img2):
    ...
    return {
        "mkeypoints0": torch.from_numpy(pts1).to(device).float(),
        "mkeypoints1": torch.from_numpy(pts2).to(device).float(),
        "match_confidence": torch.from_numpy(sift_mconf).to(device).float(),
    }
...
def find_sift_match2(torch_img1, torch_img2):
    ...
    return {
        "mkeypoints0": torch.from_numpy(np.array(pts1)).to(device).float(),
...
def opencv_pose_estimation(torch_img1, torch_img2, torch_K1, torch_K2):
    ...
    return {
        "mkeypoints0": torch.from_numpy(np.asarray(pts1)).to(device).float(),
        "mkeypoints1": torch.from_numpy(np.asarray(pts2)).to(device).float(),
        "match_confidence": torch.from_numpy(np.asarray(mconf)).to(device).float(),
    }, torch.from_numpy(R_est.reshape(1, 3, 3)).to(device).float()

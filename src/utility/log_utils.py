...
def disp_to_mpimg(disp, colormap="jet_r"):
    ...
    img = cv2.resize(fig2img(fig), (w, h))
    plt.close()
...
def rectify_images(img_ref, img_follow, K_left, dist_left, K_right, dist_right, R, t):
    height, width = img_ref.shape[:2, ...]
    R1, R2, P1, P2, Q, roi_left, roi_right = cv2.stereoRectify(
        K_left, dist_left, K_right, dist_right, (width, height), R, t, flags=cv2.CALIB_ZERO_DISPARITY
    )
    ...
    im_ref_rect = cv2.remap(img_ref, maplx, maply, cv2.INTER_LINEAR)
    im_follow_rect = cv2.remap(img_follow, maplx, maply, cv2.INTER_LINEAR)

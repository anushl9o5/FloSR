...
import model
import dataset
from utility.data_utils import calibrate_stereo_pair,calibrate_stereo_pair_torch, draw_epipolar_lines, gram_schmidt, svd_orthogonalize, undistortrectify, undistortrectify_cv2, estimate_pose_kornia, find_sift_match, make_matching_plot_fast, opencv_pose_estimation, magsac_pose_estimation
...
class Inference(BaseTrainer):
    ...
    def __init__(self, options) -> None:
        ...
        dataset_dict = {
            "torc": dataset.StereoPoseDatasetSEQ, #dataset.StereoPoseDatasetSOTA, dataset.StereoPoseDataset, #
            "carla": dataset.CarlaStereoPoseDataset, #dataset.CarlaStereoPoseDatasetSOTA,
    ...
    def save_images(self, inputs, outputs):
        ...
        pred_rotation = outputs["rotation"].detach().cpu()
        gt_rotation = inputs["rotation"].detach().cpu()
        vit_rotation = inputs["R_vit"].detach().cpu()
        ...
        vit_rmat = vit_rotation.numpy().astype(np.float32)
        translation = inputs['translation'].detach().cpu().numpy()
